...
| Accounts only where the evidence moved | the accounts step skips a theme whose fingerprint (name, gist, sorted live claim ids — `summary.fingerprint`) is unchanged; the line says "Wrote 5 of 12 theme accounts — 7 unchanged"; ACCOUNT thinks medium | `jobs._accounts`, `account.fingerprint` |
