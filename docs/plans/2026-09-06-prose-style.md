...
| `account.md` worked example | "Leaving is given as a matter of work, and the trade that paid was a small one — a stall, a yard, a cousin's shop [..]. The claims divide on whether that trade was chosen or fallen into…" | Re-worked on a theme outside any evaluation corpus, e.g. *Handover as where errors are caught*. **Correction, found in implementation:** an earlier draft of this row opened "In four of the six materials…", which breaks ACCOUNT's own rule 6 — the count is printed beside the account and the prose may not restate it. The example must state scope the way rule 6 allows ("the materials that carry this theme"), or the prompt would teach a violation of the rule three lines above it. A worked example that disobeys its own prompt is worse than the register it replaces. |
