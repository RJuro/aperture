...
{% for kind, rs in runs | groupby("kind") %}- **{{ kind }}** — {{ rs | length }} runs · {{ rs | sum(attribute="tokens_in") }} input tokens · {{ rs | sum(attribute="tokens_out") }} output tokens · {{ rs | map(attribute="provider") | unique | join(", ") }} / {{ rs | map(attribute="model") | unique | join(", ") }}
